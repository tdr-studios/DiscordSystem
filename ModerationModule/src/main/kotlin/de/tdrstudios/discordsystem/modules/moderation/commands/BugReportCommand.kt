...
class BugReportCommand : Command() {
    ...
    override fun execute(commandSender: CommandSender?, args: Array<out String>?) {
        ...
        if (args!!.size > 1) {
            ...
            if (guildData.has("moderation.bugreports.channel")) {
                var long: Long = guildData.getLong("moderation.bugreports.channel")
                var textChannel: TextChannel? = sender.guild.getTextChannelById(long)
                if (textChannel != null) {
                    var builder: EmbedBuilder = EmbedBuilder()
                    builder.setColor(Color.ORANGE)
                    builder.setDescription("Bug found by "+ sender.author.asMention);
                    ...
                    builder.addField("Bug:", sBuilder.toString(), false)
                    builder.setThumbnail(sender.author.avatarUrl)
                    textChannel.sendMessage(builder.build()).queue()
                }else sender.sendMessage(EmbedBuilder().setColor(Color.RED).setDescription("The bugreportChannel is not set! Please use "+ Discord.getPrefix(guildId) +"moderationsetup to set it")).queue()
            }else {
                sender.sendMessage(EmbedBuilder().setColor(Color.RED).setDescription("The bugreportChannel is not set! Please use "+ Discord.getPrefix(guildId) +"moderationsetup to set it")).queue()
            }
        }else{
